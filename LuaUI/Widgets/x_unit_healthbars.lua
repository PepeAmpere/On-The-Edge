...
do
  ...
  local brightClr = {}
  function DrawUnitBar(offsetY,percent,color,boss,bossName)
    if (barShader) then
...
do
  ...
  function DrawUnitInfos(unitID,unitDefID, ud)
    if not customInfo[unitDefID] then
	  ...
	  if ud.maxFuel > 0.1 then
		customInfo[unitDefID].maxFuel = ud.maxFuel
	  end
...
do
  ...
  function DrawFeatureInfos(featureID,featureDefID,fullText,fx,fy,fz)
	...
	if (maxHealth and featureIsWreck[featureID]) then maxHealth = maxHealth * 5 end --!! just hotfix, how ugly PEPE!
